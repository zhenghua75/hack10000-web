...
class ArticleController extends AdminController {
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        $signcnt=D('event_signup')->group('event_id')->getfield('event_id,count(uid)');
        trace($position);
    ...
    public function eventsignuplist(){
        ...
        if(!empty($begin) && !empty($end)){
            $map['created']=array(array('egt',$begin),array('elt',$end),'and');
        }elseif(!empty($begin)){
            $map['created']=array('egt',$begin);
        }elseif(!empty($end)){
            $map['created']=array('elt',$end);
        }
        $signlist=D('event_signup')->where($map)->select();
        foreach ($signlist as $key => &$value) {
            ...
            $exlextra=array();
            $flag=true;
            foreach ($extra as $key1 => $value1) {
                if(!array_key_exists($value1->field, $signfield)){
                    if($value1->type=='checkboxlink'){
                        $signfield[$value1->field]=$value1->linktitle;
                        array_push($signexlfield, strval($value1->linktitle));
                    }else{
                        $signfield[$value1->field]=$value1->title;
                        array_push($signexlfield, strval($value1->title));
                    }
                }
                if($value1->type=='chinacity'){
                    $area='';
                    if(!empty($province) && $province!='0'){
                        $area=getchinacityname($province);
                    }
                    if(!empty($city) && $city!='0'){
                        $area.=getchinacityname($city);
                    }
                    if(!empty($district) && $district!='0'){
                        $area.=getchinacityname($district);
                    }
                    $ascarea=mb_convert_encoding($value1->value, "ASCII");
                    if(!empty($area) && stripos($value1->value,$area)===false){
                        $flag=false;
                        unset($signlist[$key]);
                        break;
                    }
                }
                if($value1->type=='checkboxlist'){
                    if(!empty($conditions)){
                        foreach ($conditions as $cdkey => $cdvalue) {
                            if(!in_array($cdvalue, $value1->value)){
                                $flag=false;
                                unset($signlist[$key]);
                                break;
                            }
                ...
                $newextra[$value1->field]=$value1->value;
                if($value1->type=='checkboxlist'){
                    array_push($exlextra, $value1->value);
                }else{
                    array_push($exlextra, strval($value1->value));
                }
            }

            if($flag){
                $value['extra']=$newextra;
                $value['makername']=get_maker_name($value['uid']);
                $value['statusname']=$value['status']==1 ? '进行中' : '已完成';
                array_push($exlextra, strval($value['statusname']));
                array_push($exlextra, strval(date('Y-m-d H:i:s',$value['created'])));
                array_push($signexldata, $exlextra);
            }else{
                continue;
            }
        }
        array_push($signexlfield, '状态');
        array_push($signexlfield, '创建时间');
        ...
        foreach ($signexldata as $keyl => $valuel) {
            foreach ($valuel as $keyl1 => $valuel1) {
                if(is_array($valuel1)){
                    $temp='';
                    foreach ($valuel1 as $keyl2 => $valuel2) {
                        $temp=$temp.'['.$valuel2.']';
                    }
                    $signexldata[$keyl][$keyl1]=$temp;
                }
            }

        }
        $param=array('province'=>$province, 'city'=>$city, 'district'=>$district);
        $this->assign('param',$param);
        $this->assign('conditions',$conditions);
        $this->assign('begintime',$begintime);
        $this->assign('endtime',$endtime);
        $this->assign('data', $data);
        $this->assign('signfield', $signfield);
        $this->assign('signlist', $signlist);
        session('exltitle',$signexlfield);
        session('exldata',$signexldata);
        $this->meta_title   =   '报名情况';
        $this->display();
    }

    public function expEventsignup(){//导出Excel
        $xlsCell  = session('exltitle');
        $xlsData  = session('exldata');
        if(empty($xlsCell) || empty($xlsData)){
            $this->error('没有需要导出的数据');
        }else{
            ...
            $objProps->setCreator('www.hack10000.com');

            $objPHPExcel->getActiveSheet()->setTitle('活动报名表');
            $objPHPExcel->getActiveSheet(0)->mergeCells('A1:'.$cellName[$cellNum-1].'1');
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1','报名情况表');
            $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
            $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(18);
            $objPHPExcel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

            for($i=0;$i<$cellNum;$i++){
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($cellName[$i].'2', $xlsCell[$i]);
            }
            $objPHPExcel->getActiveSheet()->getStyle('A2:'.$cellName[$cellNum-1].'2')->getFont()->setSize(12);
            $objPHPExcel->getActiveSheet()->getStyle('A2:'.$cellName[$cellNum-1].'2')->getFont()->setBold(true);

            $objPHPExcel->getActiveSheet()->getStyle('A3:'.$cellName[$cellNum-1].($dataNum-1+3))->getNumberFormat()->setFormatCode("@");

             // Miscellaneous glyphs, UTF-8
            for($i=0;$i<$dataNum;$i++){
                for($j=0;$j<$cellNum;$j++){
                    $objPHPExcel->getActiveSheet(0)->setCellValue($cellName[$j].($i+3), $xlsData[$i][$j]);
                }
            }
    ...
    public function sendprivatemsg(){
        ...
        if(IS_POST){
            ...
            foreach ($to_uid as $key => $value) {
                ...
                $tmp['status']='0';
                $tmp['created']=time();
                $tmp['roleid']=get_maker_roleid($value);
                array_push($data, $tmp);
            }
            D('private_message')->addAll($data);
            $url=U('Article/eventsignuplist?id=8');
