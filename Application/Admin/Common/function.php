 ...
 *转换省市区
 */
function convert_province_string($province,$city,$county){
    $map['id']=array('in',$province.','.$city.','.$county);
    $list = D('chinacity')->where($map)->field('name')->order('level')->select();
    $strprov=$list[0]['name'].$list[1]['name'].$list[2]['name'];
    return $strprov;
}

function getchinacityname($id){
    $map['id']=$id;
    $name = D('chinacity')->where($map)->getField('name');
    return $name;
}

function get_maker_name($uid){
    $role=D('member_role')->where(array('uid'=>$uid,'status'=>'1','roleid'=>array('in','1,2')))->find();
    if($role['roleid']=='1'){
        return D('member_info')->where(array('uid'=>$uid,'roleid'=>$role['roleid'],'name'=>'truename','status'=>'1'))->getField('value');
    }else{
        return D('member_info')->where(array('uid'=>$uid,'roleid'=>$role['roleid'],'name'=>'company','status'=>'1'))->getField('value');
    }
}

function get_maker_roleid($uid){
    return D('member_role')->where(array('uid'=>$uid,'status'=>'1','roleid'=>array('in','1,2')))->getField('roleid');
}