 ...
 * 
 */
function list_to_tree_haveroot($list, $pk='id', $pid = 'pid', $child = '_child', $root) {
    // 创建Tree
    $tree = array();
    if(is_array($list)) {
        // 创建基于主键的数组引用
 ...
 *获取图片路径
*/
function get_picture_path($pid,$type,$size){
    $path='';
    if($pid){
        $picture = M('Picture')->where(array('id'=>$pid))->find();
        $servercode=substr($picture['configname'], 0, 1);
        $serverdomain=C('IMAGE_SERVER_DOMAIN')[$servercode];
        $filename=explode('.', $picture['new_filename']);
        $path='http://'.$serverdomain.'/'.strtolower($type).'/'.$filename[0].'/'.$size.'/'.$picture['new_filename'];
    }
    return $path;
}
...
function get_product_class(){
    $picture = M('product_catalog')->where(array('status'=>'1'))->order('parent_id','sort')->select();
    $servercode=substr($picture['configname'], 0, 1);
    $serverdomain=C('IMAGE_SERVER_DOMAIN')[$servercode];
    $filename=explode('.', $picture['new_filename']);
    $path='http://'.$serverdomain.'/'.strtolower($type).'/'.$filename[0].'/'.$size.'/'.$picture['new_filename'];
    ...
    foreach ($dirlist as $key => $value) {
        if($value=='.' || $value=='..'){
            ...
            if(is_dir($curdir.'/'.$value)){
                $tmpfile=hostdirscan($curdir.'/'.$value);
                array_push($filelist, array('name'=>$value,'type'=>'目录','size'=>'', 'child'=>$tmpfile));
            }else{
                $cursize=(filesize($curdir.'/'.$value))/1024;
                $cursize=round($cursize,3);
                array_push($filelist, array('name'=>$value,'type'=>'文件','size'=>$cursize, 'child'=>$value));
            }
        }
    }
    return $filelist;
