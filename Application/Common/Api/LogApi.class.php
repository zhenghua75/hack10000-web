...
class LogApi{
	...
	public $Log_Status = LogApi::LOG_CLOSED;
	public $DateFormat= "Y-m-d G:i:s";
	public $MessageQueue;
	 
	private $filename;
	private $log_file;
	private $priority = LogApi::INFO;
	...
	public function __construct()
	{
		...
		if ($config['LOG_LEVEL'] == LogApi::OFF ) return;

		$this->filename = date('Y-m-d', time()) . '.log';	//默认为以时间＋.log的文件文件
		$this->log_file = $this->createPath($config['LOG_FILE_PATH'], $this->filename);
		$this->MessageQueue = array();
		$this->priority = $config['LOG_LEVEL'];
		...
		if ( !file_exists($filepath) )	//判断文件路径是否存在
		{
			if(!empty($filepath))	//判断路径是否为空
			{
				...
				{
					$this->Log_Status = LogApi::OPEN_FAILED;
		...
		{
			$this->Log_Status = LogApi::LOG_OPEN;
	...
	public function LogInfo($line)
	{
		...
		$this->Log( $line, LogApi::INFO, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogDebug($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, LogApi::DEBUG, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogWarn($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, LogApi::WARN, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogError($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, LogApi::ERROR, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	public function LogFatal($line)
	{
		...
		$sGetFilePath = $sAarray[0]["file"];
		$sGetFileLine = $sAarray[0]["line"];
		$this->Log( $line, LogApi::FATAL, $sGetFilePath, $sGetFileLine);
		unset($sAarray);
		unset($sGetFilePath);
		unset($sGetFileLine);
	}
	...
	{
		if ( $this->Log_Status == LogApi::LOG_OPEN && $this->priority != LogApi::OFF )
		{
