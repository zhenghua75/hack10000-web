...
class Upload {
    ...
    **/
     ...
     */
    public function __construct($config = array(), $driver = '', $driverConfig = null, $size, $imgtype, $ischekform=1){
        $this->ischek=$ischekform;
     ...
     */
    public function upload($files='',$imgtype='') {
        if('' === $files){
            $files  =   $_FILES;
        }
        if(empty($files)){
        ...
        foreach ($files as $key => $file) {
            ...
            $newprefix=explode('.', $file['savename']);
            /* 调用回调函数检测文件是否存在 */
            $data = call_user_func($this->callback, $file, $this->imgtype);
            if( $this->callback && $data ){
                $info[$key] = $data;
                continue;
                // $childpath=$newprefix[0].'/';
                // if ($this->uploader->checkChildPath($this->savePath,$childpath)) {
                //     $info[$key] = $data;
                //     continue;
            ...
            $orgTempFileName=$file['tmp_name'];
            /* 检测并创建子目录 */
            $sizeconf=explode('|', $this->picsize[strtoupper($this->imgtype)]);
            if(count($sizeconf)==1){
                $this->subName=$newprefix[0].'/'.$sizeconf[0].'/';
                ...
                foreach ($sizeconf as $key1 => $value1) {
                    $this->subName=$newprefix[0].'/'.$value1.'/';
                    $subpath = $this->getSubPath($file['name']);
                    if(false === $subpath){
                        continue;
                    } else {
                    ...
                    if($value1!='org'){
                        $sizewh=explode('x', $value1);
                        if(count($sizewh)==2){
                            $image = new \Think\Image();
                            $file['tmp_name'] = '../Runtime/Temp/' . md5(time().rand()).'.jpg';
                            $image->open($orgTempFileName);
                            if(strtoupper($imgtype)=="PRODUCTDETAIL"){
                                $width = $image->width();
                                $height = $image->height();
                                $rate = $width/$sizewh[0];
                                $height = ceil($height/$rate);
                                $image->thumb($sizewh[0], $height)->save($file['tmp_name']);
                            }else{
                                $image->thumb($sizewh[0], $sizewh[1])->save($file['tmp_name']);
                            }
                        }
                    }
    ...
    private function check($file) {
        ...
        /* 检查是否合法上传 */
        if ($this->ischek && !is_uploaded_file($file['tmp_name'])) {
            $this->error = '非法上传文件！';
            return false;
        }
